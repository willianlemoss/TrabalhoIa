    ...
    while improved:
        ...
        for i in range(1, len(route) - 2):
            for j in range(i + 1, len(route)):
                ...
                if verificaCusto(new_route) < verificaCusto(best):
                    best = new_route
                    improved = True
        route = best
    return best
...
def basic_shapes(nome, vetor, dwg, posi, custo):
    ...
    while i < len(vetor) -1:
        ...
        shapes.add(text)
        i += 1
    text = dwg.text(nome, insert=(posi * mm, 120 * mm), fill = cor, font_size=15)
    shapes.add(text)
    custo = round(custo, 2)
    dia = round((custo/600), 2)
    dinheiro = round((custo / 600) * 150, 2)
    text = dwg.text(str(custo), insert=(posi * mm, 130 * mm), fill=cor, font_size=15)
    shapes.add(text)
    text = dwg.text(str(dia), insert=(posi * mm, 140 * mm), fill=cor, font_size=15)
    shapes.add(text)
    text = dwg.text(str(dinheiro), insert=(posi * mm, 150 * mm), fill=cor, font_size=15)
    shapes.add(text)
    return custo, dia, dinheiro

def total(dwg, nome1, nome2, nome3, nome4, posi):
    shapes = dwg.add(dwg.g(id='shapes', fill='black'))
    text = dwg.text(nome1, insert=(posi * mm, 120 * mm), font_size=15)
    shapes.add(text)
    text = dwg.text(nome2, insert=(posi * mm, 130 * mm), font_size=15)
    shapes.add(text)
    text = dwg.text(nome3, insert=(posi * mm, 140 * mm), font_size=15)
    shapes.add(text)
    text = dwg.text(nome4, insert=(posi * mm, 150 * mm), font_size=15)
    shapes.add(text)
...
dwg = svgwrite.Drawing(filename='percuso' + str(quantCidades)  + str(quantCaxeiros)  + '.svg', size=(300 * mm, 155 * mm), debug=True)
posi = 40
...
dwg = svgwrite.Drawing(filename='percusoT' + str(quantCidades) + str(quantCaxeiros)  + '.svg', size=(300 * mm, 155 * mm), debug=True)
posi = 40
total(dwg, '', 'Custo', 'Dia', 'Dinheiro', 10)
custo = 0
dia = 0
